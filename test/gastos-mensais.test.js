...
fs.writeFileSync(path.join(dataDir, 'db.json'), JSON.stringify({
  turmaAtiva: 'Estágio I', alunos: {}, professor: admin, professores: { 'admin-gastos': admin }, pecas: {}, proximoNum: 1, entregas: {}, sessoes: {},
  turmas: { t1: { id: 't1', nome: 'Turma teste', professores: ['admin-gastos'], criadaEm: Date.now() } }, proximaTurma: 2,
  gastos: { '2024-01': { 'prof:admin-gastos': { nome: 'Administrador de teste', tipo: 'Administrador(a)', turma: '', chamadas: 26, entrada: 300000, saida: 95869, usd: 1.49 } } }
}), 'utf8');
...
async function executar() {
  for (let i = 0; i < 80; i++) { try { if ((await fetch(base)).ok) break; } catch {} await new Promise(resolve => setTimeout(resolve, 100)); if (i === 79) throw new Error('Servidor não iniciou.\n' + log); }
  ...
  r = await fetch(base + '/api/gastos', { headers: { authorization: 'Bearer ' + login.token } });
  assert.equal(r.status, 200); const dados = await r.json(); const registro = dados.gastos['2024-01']['prof:admin-gastos'];
